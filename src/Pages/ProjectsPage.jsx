    ...
    summary:
    "This is an example text. Short summary about the current portfolio build.",
    ...
    "Future: G.R.I.D.G.X.L.Y voice assistant + secret routes.",
    ],
    },
    {
    slug: "voyagoai",
    ...
    summary:
    "Placeholder description for VoyagoAI. Trip logging + smart recs based on history.",
    ...
    summary:
    "Placeholder description. Financial literacy for immigrant & bilingual communities.",
    stack: [
    "React",
    "Vite",
    "Tailwind CSS",
    ...
    <div
    className={`mx-4 rounded-xl border border-white/10 bg-gradient-to-br from-slate-800 ${c.glow} to-transparent`}
    >
    <a
    href={p.liveUrl}
    target="_blank"
    ...
    {p.stack.map((t) => (
    <TechChip key={t} label={t} color={p.color} />
    ))}
    {p.apis.map((t) => (
    <TechChip key={t} label={t} color={p.color} />
    ))}
    ...
    />
        ...
        <div className="flex flex-wrap gap-2">
        {project.stack.map((t) => (
        <TechChip key={t} label={t} color={project.color} />
        ))}
        {project.apis.map((t) => (
        <TechChip key={t} label={t} color={project.color} />
        ))}
