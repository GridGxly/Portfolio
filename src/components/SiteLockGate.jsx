...
return (
    ...
    >
    <img src={SFX.noNo} alt="no no" width={140} height={78} style={{ display: "block", borderRadius: 8 }} />
    </div>
    );
}
...
const yayRef = useRef(null);
    ...
    if (typeof local === "string" && local.length) ok = pass.trim() === local;
    }

    if (ok) {
    window.GRIDGXLY_OVERLAYS?.burst("right", 14);
    try { yayRef.current.volume = 0.8; await yayRef.current.play(); } catch {}
    await sleep(1200);
    setUnlocked(true);
    } else {
    window.GRIDGXLY_OVERLAYS?.burst("wrong", 12);
    try { wrongRef.current.volume = 0.8; await wrongRef.current.play(); } catch {}
    await sleep(1200);
    }
    ...
    <BrainrotBackdrop />
        ...
        <div style={{ fontWeight: 700, fontSize: 22, lineHeight: 1.3, marginBottom: 18 }}>
            my portfolio is currently still being worked on, please come back later... (I appreciate the visit though, means alot❤️) Have a great rest of your day!!!!!!!
        </div>
        ...
        <BrainrotPlayer
            ...
            onSongEnded={() => {}}
            onTungCue={() => {
              const x = window.innerWidth / 2, y = Math.max(80, window.innerHeight * 0.12);
            window.GRIDGXLY_OVERLAYS?.show({ src: SFX.tungPop, x, y, size: 260, durationMs: 2400 });
            }}
        />
        </div>
    </div>
